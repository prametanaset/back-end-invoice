...
func (u *authUC) Login(username, password string) (string, string, error) {
	...
	// สร้าง access token
	accessToken, err := middleware.GenerateJWTWithExpiry(u.jwtSecret, user.ID, user.Username, user.Role, u.jwtIssuer, u.jwtAudience, u.accessExpiry)
	if err != nil {
		return "", "", err
	}
	// สร้าง refresh token (random string หรือ JWT ก็ได้ ในที่นี้ใช้ JWT ง่าย ๆ)
	refreshToken, err := middleware.GenerateJWTWithExpiry(u.jwtSecret, user.ID, user.Username, user.Role, u.jwtIssuer, u.jwtAudience, u.refreshExpiry)
	if err != nil {
		return "", "", err
	}
...
func (u *authUC) RefreshAccessToken(oldRefreshToken string) (string, string, error) {
	...
	user := &existing.User
	newAccess, err := middleware.GenerateJWTWithExpiry(u.jwtSecret, user.ID, user.Username, user.Role, u.jwtIssuer, u.jwtAudience, u.accessExpiry)
	if err != nil {
		return "", "", err
	}
	newRefresh, err := middleware.GenerateJWTWithExpiry(u.jwtSecret, user.ID, user.Username, user.Role, u.jwtIssuer, u.jwtAudience, u.refreshExpiry)
	if err != nil {
		return "", "", err
	}
